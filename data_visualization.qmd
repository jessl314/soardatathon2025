...
# panel.background = element_rect(fill = my_background_color), 
# panel.grid.major = element_line(color = my_background_color), 
#panel.grid.minor = element_line(color = my_background_color),

``` {r}
library(ggplot2)
library(dplyr)
merged_df <- left_join(cleaned_medincrecords, cleaned_medincrecords, by = "Name")
income_vs_crime_plot <- ggplot(merged_df, aes(x = `Median Household Income`, y = `Median Household Income`)) +
